...
if __name__ == "__main__":
    ...
    weight_options  = np.arange(-0.05, 0.05, 0.02)
    best_alpha = None
    best = 0.0

    # arg.alpha = [1.7, 1, 0.6, 0.6, 1.5]
    # arg.alpha = [1.7, 0,0,0, 1.5]
    alpha_init = [1, 1.2, 0.1, 0, 1.2, 1, 1.3, 0.1, 0, 0.9, 1, 0.0, 0.0, 0, 0]


    save_ensemble_ans_file = arg.save_ensemble_ans_file
    for alpha_combination in product(weight_options, repeat=15):
        arg.alpha = [x+y for x,y in zip(alpha_combination, alpha_init)]
    ...
    print(f'Best Top1 Acc: {best:.4f} with weights {best_alpha}')
