...
class Model2(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=6,
        ...
        A = np.concatenate((self.graph.A, self.graph.A),axis=0)
        self.num_class = num_class

        self.data_bn = nn.BatchNorm1d(num_person * in_channels * num_point)

        mid_channels = 64 # origin 64
        trans_channels = 128
        out_channels = 256

        #3D-TA all
        if 1:
            self.l1 = TCNC_GCN_unit(in_channels, mid_channels, A, residual=False, adaptive=adaptive, attention=attention)
            self.l2 = TCNC_GCN_unit(mid_channels, mid_channels, A, adaptive=adaptive, attention=attention)
            self.l3 = TCNC_GCN_unit(mid_channels, mid_channels, A, adaptive=adaptive, attention=attention)
            self.l4 = TCNC_GCN_unit(mid_channels, mid_channels, A, adaptive=adaptive, attention=attention)
            self.l5 = TCNC_GCN_unit(mid_channels, trans_channels, A, stride=2, adaptive=adaptive, attention=attention)
            self.l6 = TA_GCN3D_unit(trans_channels, trans_channels, 150, A, adaptive=adaptive, attention=attention,inherent=0,head=1,dropout=0.0,pe=0)
            self.l7 = TA_GCN3D_unit(trans_channels, trans_channels, 150, A, adaptive=adaptive, attention=attention,inherent=0,head=1,dropout=0.0,pe=0)
            self.l8 = TCNC_GCN3D_unit(trans_channels, out_channels, A, stride=2, adaptive=adaptive, attention=attention)
            self.l9 = TA_GCN3D_unit(out_channels, out_channels, 75, A, adaptive=adaptive, attention=attention,inherent=0,head=1,dropout=0.0,pe=0)
            self.l10 = TA_GCN3D_unit(out_channels, out_channels, 75, A, adaptive=adaptive, attention=attention,inherent=0,head=1,dropout=0.0,pe=0)
            output_emb = out_channels
        ...
        self.emb_dim = num_point * output_emb
