...
def mixup(data, label, alpha=1.0):
...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            # get data
            if epoch >= 60:
                data, label, label_mix, lam = mixup(data,label)
                data = Variable(data.float().cuda(self.output_device), requires_grad=False)
                label = Variable(label.long().cuda(self.output_device), requires_grad=False)
                label_mix = Variable(label_mix.long().cuda(self.output_device), requires_grad=False)
            else:
                data = Variable(data.float().cuda(self.output_device), requires_grad=False)
                label = Variable(label.long().cuda(self.output_device), requires_grad=False)
            ...
            if isinstance(output, tuple):
                ...
                l1 = 0
            if epoch >= 60:
                loss = lam * self.loss(output, label) + (1 - lam) * self.loss(output, label_mix) + l1
            else:
        ...
        for ln in loader_name:
            ...
            if save_score:
                with open('{}/epoch{}_{}_score.pkl'.format(
                        self.arg.work_dir, epoch, ln), 'wb') as f:
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            self.print_log(f'Epoch number: {self.best_idx}')
